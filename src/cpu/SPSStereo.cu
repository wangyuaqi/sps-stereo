...
void SPSStereo::computeInitialDisparityImage(const cv::Mat& leftImage, const cv::Mat & rightImage) {
	...
	DepthComputeUtil depthComputeUtil(p_1,p_2);

	int image_width = leftImage.cols;
	int image_height = leftImage.rows;
	std::pair<cv::Mat,cv::Mat> stereo_pair(cv::Mat(image_height,image_width,CV_8UC1),cv::Mat(image_height,image_width,CV_8UC1));

	cv::cvtColor(leftImage,stereo_pair.first,CV_RGB2GRAY);
	cv::cvtColor(rightImage,stereo_pair.second,CV_RGB2GRAY);

	const int dis_num = 128;

	cv::Mat disp_map = depthComputeUtil.DepthCompute(stereo_pair,disp_max,disp_min);
	cv::imwrite("disp.png",disp_map);
	const float disp_per = (disp_max-disp_min)/float(dis_num);

	for(int img_y =0;img_y<image_height;img_y++){
		for(int img_x =0;img_x<image_width;img_x++){
			initialDisparityImage_[img_y*image_width+img_x] = disp_min+disp_per*int(disp_map.at<uint8_t>(img_y,img_x));
		}
	}
