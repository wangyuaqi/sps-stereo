...
#define GPU_THREADS_PER_BLOCK_MAXWELL 64//64//128//64
...
#define WARP_SIZE	32//32//32//32//32//16//32
...
__inline__ __device__ int warpReduceMin(int val) {
    ...
    /*for(int d = 1; d < WARP_SIZE; d += 1) {
            ...
            val = min(val, shfl_xor_32(val, d));
        }*/
	return val;
}
//xiugai
__inline__ __device__ int n_warpReduceMin(int val) {
    val = min(val, shfl_xor_32(val, 1));
    val = min(val, shfl_xor_32(val, 2));
    val = min(val, shfl_xor_32(val, 4));
    val = min(val, shfl_xor_32(val, 8));
    ...
    val = min(val, shfl_xor_32(val, 64));
    /*val = min(val, shfl_xor_32(val, 4));
    val = min(val, shfl_xor_32(val, 8));
    val = min(val, shfl_xor_32(val, 16));*/
    return val;
...
__inline__ __device__ int n_warpReduceMinIndex(int val, int idx) {
    for(int d = 1; d < WARP_SIZE; d += 1) {
        int tmp = shfl_xor_32(val, d);
        int tmp_idx = shfl_xor_32(idx, d);
        if(val > tmp) {
